...
function getDefaultDatabaseUrl(projectId) {
  ...
  return FIREBASE_DATABASE_EMULATOR_HOST ? `http://${FIREBASE_DATABASE_EMULATOR_HOST}?ns=${projectId || 'local'}` : `https://${projectId}.firebaseio.com`;
}
/**
...
export function initializeFirebase(adminInstance, overrideConfig) {
  try {
    ...
    /* eslint-disable no-console */
    const dbUrlLog = fbConfig.databaseURL ? ` and databaseURL "${fbConfig.databaseURL}"` : '';
...
export function slashPathToFirestoreRef(firestoreStatics, slashPath, options) {
  ...
  if (options?.where && Array.isArray(options.where) && typeof ref.where === 'function') {
